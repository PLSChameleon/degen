...
def withdraw(token_address: address):
    ...
    current_balance: uint256 = IERC20(token_address).balanceOf(self)
    assert current_balance > 1, "Insufficient balance to withdraw"
    IERC20(token_address).transfer(self.owner, current_balance - 1)
...
def uniswapV2Call(
    ...
    assert (_amount0 > 0 and _amount1 == 0) or (_amount0 == 0 and _amount1 > 0), "Invalid flash swap amounts"
